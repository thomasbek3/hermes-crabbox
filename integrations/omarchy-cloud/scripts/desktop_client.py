...
SSH = ['/usr/bin/ssh', '-o', 'BatchMode=yes', '-o', 'ControlMaster=no',
...
def open_desktop(session_id):
    ...
    try:
        ...
        binary = Path.home() / '.local/lib/crabbox/0.61.0/crabbox'
        info = binary.lstat()
        ...
        while time.monotonic() < deadline:
            ...
            owned = subprocess.run(['/usr/sbin/lsof', '-nP', '-a', '-iTCP@127.0.0.1:' + str(port),
