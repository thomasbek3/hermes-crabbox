...
import argparse
parser=argparse.ArgumentParser();parser.add_argument('--mode',choices=['uds_drop_once','http_drop_once'],required=True);args=parser.parse_args()
root=Path(__file__).resolve().parents[1]

paths=['src/cloudworkbench/inference_transport.py','src/cloudworkbench/hermes_inference_protocol.py','src/cloudworkbench/inference_service.py','src/cloudworkbench/inference_relay.py','tests/fixtures/hermes/base-tool-schemas.json','scripts/qualify-inference-relay-native.py']
...
owner='d9relay-'+uuid.uuid4().hex[:12];name='cwb-'+owner
...
runpy.run_path(str(stage/'scripts/qualify-inference-relay-native.py'),run_name='__main__')
'''%(sources,hashes)
