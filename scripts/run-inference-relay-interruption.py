...
import argparse
parser=argparse.ArgumentParser();parser.add_argument('--mode',choices=['http_interrupt'],required=True);args=parser.parse_args()
root=Path(__file__).resolve().parents[1]

paths=['src/cloudworkbench/inference_transport.py','src/cloudworkbench/hermes_inference_protocol.py','src/cloudworkbench/inference_service.py','src/cloudworkbench/inference_relay.py','tests/fixtures/hermes/base-tool-schemas.json','scripts/qualify-inference-relay-interruption.py']
...
owner='d9interrupt-'+uuid.uuid4().hex[:12];name='cwb-'+owner
...
runpy.run_path(str(stage/'scripts/qualify-inference-relay-interruption.py'),run_name='__main__')
'''%(sources,hashes)
argv=['/usr/bin/docker','run','--name',name,'--label','io.cloudworkbench.probe-owner='+owner,'--network','none','--read-only',
 '--user','1000:1000','--cpus','2','--memory','1536m','--memory-swap','1536m','--pids-limit','128','--cap-drop','ALL','--security-opt','no-new-privileges',
 '--tmpfs','/tmp:rw,exec,nosuid,nodev,size=256m,mode=1777','--workdir','/tmp','-e','PYTHONDONTWRITEBYTECODE=1','-e','CWB_PROBE_MODE='+args.mode,
...
stamp=datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ');base=root/'evidence'/('inference-relay-native-'+args.mode+'-'+stamp)
...
try:
 result=subprocess.run(ssh+[shlex.join(argv)],input=program,capture_output=True,text=True,timeout=90)
 ...
 try:
  ...
  assert metadata['limits']['PidMode']=='' and metadata['limits']['PidsLimit']==128
  assert metadata['limits']['Memory']==1610612736 and metadata['limits']['NanoCpus']==2000000000
