...
import hashlib,importlib.metadata,json,os,socket,subprocess,sys,tempfile,threading,time
...
mode=os.environ.get('CWB_PROBE_MODE','uds_drop_once');assert mode in ('uds_drop_once','http_drop_once')
...
relay=InferenceRelay(socket_path=root/'inference.sock',journal_path=journal_root/'relay.db',binding=binding,capability='c'*64,fence=fences.append,deadline_seconds=15)
...
if mode=='uds_drop_once':
    ...
    if cli.returncode!=0 and len(terminal)==1 and terminal[0].get('exit_code')!=0:checks.append('native_http_loss_terminal_failure')
    if len(accepted)==1 and len(results)==1 and len(uds_nonces)==1:checks.append('native_http_loss_no_transparent_retry')
