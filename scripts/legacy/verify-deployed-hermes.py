...
def main(argv=None):
    ...
    require(activation.get('passed') is True and activation.get('stage')=='complete' and activation.get('execute') is True
        and activation.get('host')=='archived-worker.invalid' and activation.get('environment_sha256')==ENVIRONMENT_SHA,'completed_activation_required')
    ...
    receipt={'schema_version':1,'host':'archived-worker.invalid','script_sha256':sha(Path(__file__).read_bytes()),
