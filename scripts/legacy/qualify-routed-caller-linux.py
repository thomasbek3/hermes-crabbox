...
def validate_receipt(receipt,bundle):
    if (receipt.get('passed') is not True or receipt.get('run_id')!=bundle['run_id']
            or receipt.get('image')!=bundle['image'] or receipt.get('host')!='archived-worker.invalid'
...
def remote(bundle_path,receipt_path):
    ...
    if socket.gethostname()!='archived-worker.invalid' or os.geteuid()!=0:raise RuntimeError('wrong_qualification_host')
