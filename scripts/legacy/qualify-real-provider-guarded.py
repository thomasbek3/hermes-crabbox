...
def validate_manifest(manifest,*,now):
    ...
    if type(manifest) is not dict or set(manifest)!=expected or manifest['version']!=1 or manifest['host']!='archived-worker.invalid':
...
def main(argv=None):
    ...
    if socket.gethostname()!='archived-worker.invalid' or os.geteuid()!=0:raise QualificationRefused('verified_omarchy_root_required')
    ...
    receipt={'host':'archived-worker.invalid','executed':False,'passed':False,'real_provider_requested':args.execute_approved}
