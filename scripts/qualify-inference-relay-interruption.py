...
import hashlib,importlib.metadata,json,os,signal,socket,subprocess,sys,tempfile,threading,time
...
from cloudworkbench.inference_relay import AttemptBinding,DispatchResult,InferenceRelay,WorkerDispatcher,WorkerSocketServer
import cloudworkbench.inference_relay as relay_module

mode='http_drop_once'
controller_interrupt=True
...
native_process=None;interruptions=[]
requests=[];results=[];refusals=[];http_events=[];http_requests=[];fences=[];uds_nonces=[];dropped_uds=[];dropped_http=[]
class AuditHandler(_Handler):
    def _reply(self,status,body,content_type='application/json'):
        if mode=='http_drop_once' and self.path=='/v1/chat/completions' and status==200 and not dropped_http:
            dropped_http.append(True);self.connection.shutdown(socket.SHUT_RDWR);self.connection.close()
            raise ConnectionError('synthetic lost HTTP response')
...
def execute(body,cancel):
    ...
    try:
        ...
        return DispatchResult(ServiceResponse(response.status_code,response.content_type,response.body),True)
    except (ProtocolError,TransportError) as exc:
        status=getattr(exc,'status_code',502)
        refusals.append({'code':exc.code,'status':status,'request':body})
        return DispatchResult(ServiceResponse(status,'application/json',json.dumps({'error':{'code':exc.code,'message':exc.code}}).encode()),True)
journal_root=root/'journals';journal_root.mkdir(mode=0o700)
binding=AttemptBinding('synthetic-'+mode,1,hashlib.sha256(b'synthetic frozen profile').hexdigest())
worker=WorkerDispatcher(journal_path=journal_root/'worker.db',binding=binding,capability_sha256=hashlib.sha256(('c'*64).encode()).hexdigest(),authorize=lambda b:True,execute=execute)
uds=WorkerSocketServer(root/'inference.sock',worker,deadline_seconds=15)
original_dispatch=worker.dispatch
original_send=relay_module._send_frame
def audit_dispatch(request,cancel):
    uds_nonces.append(request['nonce']);return original_dispatch(request,cancel)
worker.dispatch=audit_dispatch
def fault_send(sock,value,deadline,cancel):
    if mode=='uds_drop_once' and isinstance(value,dict) and 'response' in value and not dropped_uds:
        dropped_uds.append(value['nonce']);sock.close();raise ConnectionError('synthetic lost UDS response')
    return original_send(sock,value,deadline,cancel)
relay_module._send_frame=fault_send
uds_thread=threading.Thread(target=uds.serve_forever,daemon=True);uds_thread.start()
def controller_fence(code):
    fences.append(code)
    if native_process is not None and native_process.poll() is None:
        os.killpg(native_process.pid,signal.SIGTERM)
        interruptions.append({'reason':code,'pid':native_process.pid,'signal':'SIGTERM','time':time.monotonic()})
relay=InferenceRelay(socket_path=root/'inference.sock',journal_path=journal_root/'relay.db',binding=binding,capability='c'*64,fence=controller_fence,deadline_seconds=15)
server=InferenceService(('127.0.0.1',0),capability_sha256=hashlib.sha256(('b'*64).encode()).hexdigest(),authorize=lambda:True,execute=relay.forward,delivery_result=relay.delivery_result,event=lambda status,code:http_events.append({'status':status,'code':code}),execution_timeout=20)
...
native_process=subprocess.Popen(argv,cwd=root,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,start_new_session=True)
stdout,stderr=native_process.communicate(timeout=50)
cli=subprocess.CompletedProcess(argv,native_process.returncode,stdout,stderr)
end=time.monotonic()+1
while time.monotonic()<end:
    try:os.killpg(native_process.pid,0)
    except ProcessLookupError:break
    time.sleep(.02)
try:
    os.killpg(native_process.pid,0);native_group_stopped=False
except ProcessLookupError:native_group_stopped=True
if not native_group_stopped:os.killpg(native_process.pid,signal.SIGKILL)
interruption_exit_delay=time.monotonic()-interruptions[0]['time'] if interruptions else None
server.shutdown();server.server_close();thread.join(2)
uds.shutdown();uds.server_close();uds_thread.join(2)
relay_rows=relay.journal.db.execute('SELECT nonce,payload_digest,state FROM requests').fetchall()
worker_rows=worker.journal.db.execute('SELECT nonce,payload_digest,state FROM requests').fetchall()
relay.close();worker.close()
events=[];invalid=[]
...
if mode=='uds_drop_once':
    if len(uds_nonces)==3 and uds_nonces[0]==uds_nonces[1] and uds_nonces[2]!=uds_nonces[0] and len(results)==2 and all(r[2]=='http_write_completed' for r in relay_rows):checks.append('lost_uds_response_same_nonce_no_duplicate_execution')
    expected=6
else:
    checks=[]
    accepted=[r for r in http_requests if r['method']=='POST' and r['path']=='/v1/chat/completions']
    if cli.returncode!=0 and interruptions and native_group_stopped:checks.append('controller_fence_stopped_actual_native_group')
    if len(accepted)==1 and len(results)==1 and len(uds_nonces)==1:checks.append('controller_interruption_before_native_retry')
    if fences==['response_delivery_unknown'] and len(relay_rows)==1 and relay_rows[0][2]=='response_delivery_unknown':checks.append('durable_http_delivery_unknown_fence')
    if len(worker_rows)==1 and worker_rows[0][2]=='cached':checks.append('worker_response_retained_without_regeneration')
    expected=4
receipt={'passed':len(checks),'failed':expected-len(checks),'mode':'http_loss_controller_interrupt','controller_interruptions':interruptions,'native_group_stopped':native_group_stopped,'interruption_exit_delay_seconds':interruption_exit_delay,'uds_nonces':uds_nonces,'dropped_uds':dropped_uds,'dropped_http':dropped_http,'relay_rows':relay_rows,'worker_rows':worker_rows,'fences':fences,'hermes_stream_retries':os.environ['HERMES_STREAM_RETRIES'],'same_uid_synthetic_only':True,'production_uid_mount_qualified':False,'checks':checks,'suite':'D9 native Hermes HTTP UDS relay synthetic worker loop and delivery faults','source_sha256':json.loads((stage/'source-manifest.json').read_text()),'cli_argv':argv,'cli_exit':cli.returncode,'config':config,'events':events,'invalid_stdout_lines':invalid,'stderr':cli.stderr[-12000:],'requests':requests,'results':results,'refusals':refusals,'http_events':http_events,'http_requests':http_requests,'hermes_version':version,'synthetic':True,'real_provider_calls':False,'real_credentials':False,'auth_lease_qualified':False,'outer_container_cleanup_required':True,'seconds':time.monotonic()-started}
