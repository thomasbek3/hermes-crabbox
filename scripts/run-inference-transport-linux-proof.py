...
binding=json.loads((root/'evidence/inference-transport-binding.json').read_text())
paths=['src/cloudworkbench/inference_transport.py','tests/fixtures/hermes/base-tool-schemas.json','scripts/qualify-inference-transport-linux.py']
...
for path in paths[:2]:assert hashes[path]==binding['files'][path],path
image='sha256:5a03c9d2d1fc20683029c47683a3b0470ad2d7ce79eeb43ced1bdfba10770693'
owner='h1linux-'+uuid.uuid4().hex[:12];name='cwb-'+owner
...
runpy.run_path(str(stage/'scripts/qualify-inference-transport-linux.py'),run_name='__main__')
'''%(sources,hashes)
...
stamp=datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ');base=root/'evidence'/('inference-transport-linux-'+stamp)
