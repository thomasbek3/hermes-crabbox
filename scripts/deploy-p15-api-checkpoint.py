...
def validate(source, base):
    ...
    if not base['passed'] or base['host']!='omarchy':raise ValueError('No successful base deployment')
...
def main():
    ...
    if os.geteuid()!=0 or socket.gethostname()!='omarchy':raise ValueError('Root on Omarchy required')
