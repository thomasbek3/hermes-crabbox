#!/usr/bin/env python3
"""Add evidence-enabled images while preserving existing pinned environments."""
...
def main():
    ...
    target=ROOT/'environments/hermes-pr-evidence-v1.db';assert not target.exists()
    additions=[]
    for base,version,image,routed in [('hermes-tasks-desktop-v1','hermes-tasks-desktop-evidence-v1',a.single_image,False),('hermes-tasks-pstack-v1','hermes-tasks-pstack-evidence-v1',a.routed_image,True)]:
    ...
    backup=ROOT/'operator-backups'/('pr-evidence-'+time.strftime('%Y%m%dT%H%M%SZ',time.gmtime()));backup.mkdir(mode=0o700)
