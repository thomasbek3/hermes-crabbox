...
paths=['src/cloudworkbench/inference_transport.py','src/cloudworkbench/hermes_inference_protocol.py','src/cloudworkbench/inference_service.py','tests/fixtures/hermes/base-tool-schemas.json','scripts/qualify-hermes-native-inference-loop.py']
...
owner='h1loop-'+uuid.uuid4().hex[:12];name='cwb-'+owner
...
runpy.run_path(str(stage/'scripts/qualify-hermes-native-inference-loop.py'),run_name='__main__')
'''%(sources,hashes)
argv=['/usr/bin/docker','run','--name',name,'--label','io.cloudworkbench.probe-owner='+owner,'--network','none','--read-only',
 ...
 '--tmpfs','/tmp:rw,exec,nosuid,nodev,size=256m,mode=1777','--workdir','/tmp','-e','PYTHONDONTWRITEBYTECODE=1',
...
stamp=datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ');base=root/'evidence'/('hermes-native-inference-loop-'+stamp)
