...
binding=json.loads((root/'evidence/provider-bootstrap-binding.json').read_text())
paths=['src/cloudworkbench/inference_transport.py','src/cloudworkbench/adapters.py','src/cloudworkbench/provider_bootstrap.py','scripts/qualify-provider-bootstrap-linux.py']
...
for path in paths[1:3]:assert hashes[path]==binding['files'][path]
for path in paths:assert (root/path).read_bytes()==sources[path].encode(), 'source encoding drift'
image='sha256:5a03c9d2d1fc20683029c47683a3b0470ad2d7ce79eeb43ced1bdfba10770693'
owner='h1bootstrap-'+uuid.uuid4().hex[:12];name='cwb-'+owner
ssh=['ssh','-o','BatchMode=yes','-o','ConnectTimeout=10','thomas@100.83.74.92']
program='''import hashlib,json,os,pathlib,runpy,sys,tempfile
sources=%r
hashes=%r
stage=pathlib.Path(tempfile.mkdtemp(prefix='h1-',dir='/tmp'))
...
runpy.run_path(str(stage/'scripts/qualify-provider-bootstrap-linux.py'),run_name='__main__')
'''%(sources,hashes)
argv=['/usr/bin/docker','run','--name',name,'--label','io.cloudworkbench.probe-owner='+owner,'--network','none','--read-only',
 ...
 '--entrypoint','/opt/hermes/venv/bin/python','-i',image,'-']
stamp=datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ');base=root/'evidence'/('provider-bootstrap-linux-'+stamp)
