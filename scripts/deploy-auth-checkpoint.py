...
def validate_qualified_resume(prior, receipt, manifest, registry, old_registry, image_id):
    if prior.get('host')!='omarchy' or prior.get('passed') is not False or prior.get('execute') is not True or prior.get('stage')!='credential_state':
...
def main(argv=None):
    ...
    if os.geteuid()!=0 or socket.gethostname()!='omarchy':raise ValueError('Only root on Omarchy may run this operator command')
