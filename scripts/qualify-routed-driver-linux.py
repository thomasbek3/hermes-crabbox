...
def prepare(output,image,*,scenario="results"):
    import uuid
    sources=source_closure(ROOT);support=(ROOT/'scripts'/SUPPORT).read_text();harness=Path(__file__).read_text()
...
def _validate_receipt(receipt,bundle):
    if (receipt.get('proof_version')!=2 or type(receipt.get('proof_version')) is not int or receipt.get('scenario')!=bundle['scenario']
            or receipt.get('passed') is not True or receipt.get('run_id')!=bundle['run_id'] or receipt.get('host')!='omarchy'
...
def remote_entry(bundle):
    ...
    validate_bundle(bundle)
    if socket.gethostname()!='omarchy' or os.geteuid()!=0:raise RuntimeError('wrong_host')
