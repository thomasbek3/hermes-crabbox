...
def support_module(value):
    module=types.ModuleType('frozen_worker_fixture');module.__file__=str(ROOT/'scripts'/SUPPORT)
...
def prepare(path):
    import secrets
    source=(ROOT/'scripts'/SUPPORT).read_text();harness=Path(__file__).read_text()
...
def validate_receipt(value,bundle):
    ...
    try:
        ...
        assert value['passed'] is True and value['host']=='omarchy' and value['image']==IMAGE
...
def root_run(bundle,receipt_path):
    ...
    if sys.platform!='linux' or os.geteuid()!=0 or socket.gethostname()!='omarchy' or Path('/tmp').resolve()!=Path('/tmp'):raise ValueError('linux_omarchy_root_required')
    ...
    result={'passed':False,'host':'omarchy','run_id':bundle['run_id'],'image':IMAGE,'source_hashes':bundle['hashes'],
