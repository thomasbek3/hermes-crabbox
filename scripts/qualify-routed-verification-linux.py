...
def driver_module(bundle=None):
    source = bundle['driver_harness'] if bundle else (ROOT / 'scripts' / DRIVER).read_text()
...
def prepare(path, mode):
    ...
    support = (ROOT / 'scripts' / driver.SUPPORT).read_text()
    harness = Path(__file__).read_text(); base = (ROOT / 'scripts' / DRIVER).read_text()
